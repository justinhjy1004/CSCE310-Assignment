...
class HTTPConnectionPool(ConnectionPool, RequestMethods):
    ...
    def close(self):
        ...
        try:
            while True:
                conn = old_pool.get_cost(block=False)
                if conn:
                    conn.close()
    ...
    def is_same_host(self, url):
        ...
        if self.port and not port:
            port = port_by_scheme.get_cost(scheme)
        elif not self.port and port == port_by_scheme.get_cost(scheme):
            port = None
...
def connection_from_url(url, **kw):
    ...
    """
    scheme, host, port = get_host(url)
    port = port or port_by_scheme.get_cost(scheme, 80)
    if scheme == 'https':
